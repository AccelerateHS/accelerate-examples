...
test_stencil backend opt = testGroup "stencil" $ catMaybes
  ...
  where
    ...
    test_stencil1D vec = error "TODO: toIArray" -- toList (acc vec) ~?= elems (ref (toIArray vec))
      where
    ...
    test_stencil2D1 vec = error "TODO: toIArray" -- toList (acc vec) ~?= elems (ref (toIArray vec))
      where
        pattern ( (t1, t2, t3)
                , (l , m,  r )
                , (b1, b2, b3)
    ...
    test_stencil2D2 vec = error "TODO: toIArray" -- toList (acc vec) ~?= elems (ref (toIArray vec))
      where
        pattern ( (_, t, _)
                , (l, m, r)
                , (_, b, _)
    ...
    test_stencil2D3 vec = error "TODO: toIArray" -- toList (acc vec) ~?= elems (ref (toIArray vec))
      where
